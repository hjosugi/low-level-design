...
Shubham Tomar
• 5 months ago
can we have a video tutorial for these as well, please ??
13
HARDIK SHARMA
HARDIK SHARMA
Top 10%
...
Top 5%
• 3 months ago
minor suggestion: the first line in checkWin to check row, col bounds could be replaced with the existing  inBounds method:
if row < 0 || row >= rows || col < 0 || col >= cols -> if not inBounds(row, col)
5
N
NakedFuchsiaRooster639
• 2 months ago
• edited 2 months ago
I had a design question while comparing this solution with my own approach.
In my version, I kept Board.makeMove(column, disc) responsible for:
validating the move
placing the disc
checking win/draw
returning a MoveResult
So Game simply tells the board to apply the move and then updates turn / winner based on the result.
In this solution, Game.makeMove(...) first calls board.placeDisc(...), then separately calls board.checkWin(...) and board.isFull().
My question is:
From an OO design perspective, which responsibility split is considered better here?
Should Board fully own move consequences because it owns board state (Tell, Don’t Ask style)?
Or is keeping Board simpler and letting Game orchestrate place + check + state update considered cleaner in interviews?
Also related: is Game.makeMove(...) generally preferred over a play() loop inside Game because interview solutions usually treat Game as a domain model rather than an executable game runner?
2
Abhinav Tripathi
Abhinav Tripathi
• 5 days ago
Board should be a simple entity which should have simple functionalities like canPlaceDisc, placeDisc, isFull, checkIf4DiscOfColor is present in row/col/diagonals (basically check for winner) and game should be a facade which orchestrate the game flow. all the business logic is present in game class and it should orchestrate other entities to make this game flow (makeMove method) work.
since this question was a simple example with only board entity involved your approach might see correct, but the moment logic becomes complex it might break. Hence, to keep the design modular and scalable for future, we should design exactly the same way @Evan has done in the article. This is my opinion and i may be wrong but i would personally prefer this.
1
Tushar Gupta
Tushar Gupta
• 28 days ago
Regarding your last point, I had the same doubt in my mind. Having a loop inside play() method would make more sense, just as this is being done in case of bot
while game.getGameState() == IN_PROGRESS
    current = game.getCurrentPlayer()
    if current == humanPlayer
        column = /* read from UI / input */
        ...
        column = bot.chooseMove(game)
    game.makeMove(current, column)
here instead of this bot.choseMove we can make changes to call for another player.
1
sanjeev kumar
sanjeev kumar
Top 5%
• 4 months ago
Lovely Video Explanation, please provide some premium video content as well, @Evan
2
Reading Progress
On This Page
Understanding the Problem
