# Common Problems: Parking Lot

**By**: Evan King  
**Published**: Dec 15, 2025  
**Medium**: Low Level Design (LLD)
...
classDiagram
    ...
    ParkingLot "1" --> "many" Ticket : creates / manages
```
...
While most companies only require pseudocode during interviews, some do ask for full implementations of at least a subset of the classes or methods. Below is a complete working implementation in Python for reference.
...
class Ticket:
    def __init__(self, ticket_id: str, spot_id: str, vehicle_type: VehicleType, entry_time: int):
        self._id = ticket_id
...
class ParkingLot:
    ...
    def _map_vehicle_type_to_spot_type(self, vehicle_type: VehicleType) -> SpotType:
...
* `findAvailableSpot(CAR)` $\rightarrow$ finds spot B (type matches, not in occupiedSpotIds)
* `occupiedSpotIds.add("B")` $\rightarrow$ set now `{"B"}`
* Generate ticket: `id="T123"`, `spotId="B"`, `vehicleType=CAR`, `entryTime=1000000`
* `activeTickets.put("T123", ticket)` $\rightarrow$ map now `{"T123" -> ticket}`
* **Return**: ticket T123
...
`exit("T123")`
* `activeTickets.get("T123")` $\rightarrow$ ticket found
* `exitTime = 1000000 + (2.5 hours in millis) = 10000000`
* `computeFee(1000000, 10000000)`:
  * duration = 2.5 hours
  * round up $\rightarrow$ 3 hours
  * fee = `3 * 500 = 1500` cents
* `occupiedSpotIds.remove("B")` $\rightarrow$ set now empty
* `activeTickets.remove("T123")` $\rightarrow$ map now empty
* **Return**: 1500 cents
...
`exit("T123")`
* `activeTickets.get("T123")` $\rightarrow$ null (already removed)
...
* `findAvailableSpot(CAR)` $\rightarrow$ returns null
